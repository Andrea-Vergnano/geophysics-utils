...
dimensions_georef_plot=150 # in m, dimensions of the georeferenced png rotate plot files. It actually comes 40% smaller for some reasons.
...
recover_coords_from_file=TRUE
utmzone=38 # for eventual UTM conversion performed when giving external coordinates with recover coords from file.
cut_char=11 #characters to be cut from filename for entitling the graph. Also, one option is to go where the composite plot is created and just put the i variable, so the name of the plots is just 1,2,3,4. In any case, the software takes the files in alphabetical order, therefore, if they are named after the date-hour, they should come out in the correct order. anyanyway, there are the coordinates that prevent any mistake positioning or confusion between points.
...
logspace_colorscale=TRUE #set TRUE if you want the color scale to be more sensitive to small values of amplitude.
...
#log10 color scale
if(logspace_colorscale==TRUE){
breaks_var=round((logspace(0, log10(max_plot_scale), 15)),1) #this length.out value should be the same of the bins in the MetR::geom_contour_fill in the ggplot function.
}
...
if(spec==TRUE)
{
  (p1+p4)/(p2+p3)+plot_annotation(title = sprintf("H/V ratio of point %s",substr(element,1,nchar(element)-cut_char)))

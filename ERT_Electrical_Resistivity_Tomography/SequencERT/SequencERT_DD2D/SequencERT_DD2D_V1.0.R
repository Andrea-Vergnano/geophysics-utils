...
write.table(sequ,sprintf("output/DD%i.csv",channels),row.names = FALSE,quote=FALSE,sep=",")
write.table(sequ_roll,sprintf("output/DD%i_roll.csv",channels),row.names = FALSE,quote=FALSE,sep=",")
...
colnames(ele_pos)[1]="#"
write.table(ele_pos,sprintf("output/DD%io.txt",channels),row.names = FALSE,quote=FALSE,sep="\t")
sequ_txt=cbind(c(1:length(sequ[,1])),sequ)
colnames(sequ_txt)[1]="#"
write.table(sequ_txt,sprintf("output/DD%io.txt",channels),row.names = FALSE,quote=FALSE,sep="\t",append = TRUE)

#write roll along sequence
write.table(ele_pos,sprintf("output/DD%ioR.txt",channels),row.names = FALSE,quote=FALSE,sep="\t")
sequ_txt_roll=cbind(c(1:length(sequ_roll[,1])),sequ_roll)
colnames(sequ_txt_roll)[1]="#"
write.table(sequ_txt_roll,sprintf("output/DD%ioR.txt",channels),row.names = FALSE,quote=FALSE,sep="\t",append = TRUE)

...
#1.4 February 2026 Added support for MAE output .tsv files.

#1.5 February 2026. Added a script to smooth the electrode elevations, when there is some elevation scattering in GNSS data.
...
library(dplyr)
library(mgcv) #to smooth the elevations of the electrodes.

### End libraries ###

##Read the .bin data file, which is the output of Syscal pro georesistivimeter
...
#Read a .tsv file which is the output of MAE georesistivimeter
raw_data=read.table(choose.files(),sep="\t",header=TRUE)
...
clean_data$Vmn=clean_data$V..mV.
clean_data$Iab=clean_data$I..mA.
clean_data$DevRho=abs(clean_data$var/clean_data$r..O.m.)*100
...
# #
# 
ele_num_to_add=48*2 #in the CSV file, to adjust the ele numbering of this survey.
...
#Simple smoothing on z
# ele_pos$z=smooth(ele_pos$z)


#More advanced smoothing: with a 3D surface using generalized additive models:

##Fitting with a no-forumla generalized additive model:

model <- gam(z ~ s(x, y), data = ele_pos)
z_smoothed=model$fitted.values


#2d plot of the results

ggplot()+
  geom_point(aes(1:length(ele_pos$z),ele_pos$z))+
  geom_point(aes(1:length(z_smoothed),z_smoothed),color="red")

#3d plot of the results:

fig <- plot_ly(x = ele_pos$x, y =ele_pos$y, z = ele_pos$z,
               type = 'scatter3d', mode = 'markers')
##add electrode positions
fig <- fig %>%
  add_markers( x = ele_pos$x, y = ele_pos$y,z = z_smoothed, marker=list(color="red",symbol="square"))
fig


#if smoothed positions seems OK, save them on ele_pos$z
ele_pos$z=z_smoothed

#Update the electrode positions based on the imported .csv topographic file.
...
clean_data$Rho=clean_data$V..mV./clean_data$I..mA.*clean_data$k
...
plotDevRho=ggplot() + 
  ...
  ggplot2::ggtitle(sprintf("%s < Dev Rho < %s",round(min(abs(clean_data$DevRho)),3),round(max(clean_data$DevRho),3)))+
